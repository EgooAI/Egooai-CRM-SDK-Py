...
class SessionMetaManagerTestCase(unittest.TestCase):
    ...
    def test_list_session_meta_returns_all_records_in_sid_order(self) -> None:
        ...
        second = SessionMeta(name="session-b", participants=[3, 4, 5])
    ...
    def test_edit_session_meta_updates_fields(self) -> None:
        session_meta = self._build_session_meta()
        self.manager.add_session_meta(session_meta)
    ...
    def test_edit_session_meta_raises_when_missing(self) -> None:
        updated_session_meta = SessionMeta(name="ghost-session", participants=[0])

        with self.assertRaises(ValueError):
            self.manager.edit_session_meta(404, updated_session_meta)

    def test_delete_session_meta_removes_record(self) -> None:
        session_meta = self._build_session_meta()
        self.manager.add_session_meta(session_meta)
    ...
    def test_upsert_session_meta_inserts_when_missing(self) -> None:
        session_meta = self._build_session_meta(name="session-upsert")

        self.manager.upsert_session_meta(session_meta)
